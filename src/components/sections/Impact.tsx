...
const Impact = () => {
    return (
        <section className="bg-gradient-to-b from-white via-green-50/20 to-white py-40 relative overflow-hidden border-t border-green-100/50">
            ...
            {/* Stats Cards */}
            <div className="max-w-[95rem] mx-auto px-8 relative z-20">
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {HUILA_STATS.map((stat, i) => {
                        const isCompromiso = stat.label === "Compromiso Huila";
                        return (
                            <div key={i} className={`group relative card-modern hover:-translate-y-3 border-2 transition-all duration-300 overflow-hidden ${
                                isCompromiso 
                                ...
                                <div className="relative z-10">
                                    <div className={`flex items-baseline mb-6 ${isCompromiso ? 'gap-2 md:gap-3' : 'gap-2'} flex-nowrap`}>
                                        <p className={`font-display text-green-600 group-hover:text-green-700 transition-colors leading-none tracking-tight whitespace-nowrap ${
                                            isCompromiso 
                                        ...
                                        {stat.value.includes('%') && (
                                            <span className={`font-display text-green-600 group-hover:text-green-700 transition-colors leading-none ${
                                                isCompromiso ? 'text-5xl md:text-6xl lg:text-7xl' : 'text-5xl md:text-6xl'
                                            }`}>%</span>
                                        )}
                                    </div>
                                    <div className={`bg-green-600 rounded-full mb-6 group-hover:w-full transition-all duration-500 ${
                                        isCompromiso ? 'w-28 md:w-32 h-1.5 md:h-2' : 'w-24 h-1.5'
                                    }`}></div>
                                    <p className={`font-bold uppercase tracking-[0.4em] text-green-700 group-hover:text-green-800 transition-colors leading-tight ${
                                        isCompromiso ? 'text-sm md:text-base' : 'text-xs md:text-sm'
                                    }`}>
                                        {stat.label}
                                    </p>
                                </div>
